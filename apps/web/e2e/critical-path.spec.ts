...
test.describe('Golden Path — Alta de paciente + Agenda de turno', () => {
  test('crea un paciente pediátrico y registra su turno en la agenda', async ({
    page,
  }) => {
    ...
    // Tutor responsable
    await page
      .getByTestId('patient-guardianFullName')
      .fill(`Tutor ${firstName}`);
    ...
    await expect(
      page.getByRole('heading', { name: new RegExp(patientName) }),
    ).toBeVisible();
    ...
    await page
      .getByTestId('appointment-patient-select')
      .selectOption({ label: patientName });
    ...
    await page
      .getByRole('button', { name: dayPickerAriaLabel(slot.date) })
      .click();
    ...
    await expect(page.getByTestId('appointment-time')).toContainText(
      /seleccion/i,
      { timeout: 10_000 },
    );
    ...
    await page
      ...
      .fill('Turno generado por suite E2E Playwright.');

    // Submit → toast de éxito → el dialog se cierra.
    await page.getByTestId('appointment-submit').click();
    await expect(page.getByTestId('appointment-toast')).toContainText(
      /turno agendado con éxito/i,
    );
    ...
    if (slot.isToday) {
      ...
      await expect(page.getByTestId('todays-bookings-table')).toContainText(
        patientName,
      );
    }
  });
